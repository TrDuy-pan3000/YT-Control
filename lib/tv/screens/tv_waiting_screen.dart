...
class _TvWaitingScreenState extends State<TvWaitingScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _setupServerAndDiscovery() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Lỗi khởi động Server: $e'), backgroundColor: AppColors.error),
        );
      }
    }
    ...
    // 4. Lắng nghe trạng thái kết nối của Client
    wsServer.addListener(_onServerStateChange);
  }

  void _onServerStateChange() {
    final wsServer = Provider.of<WsServerService>(context, listen: false);
    if (wsServer.isClientConnected && mounted) {
      // Đã có Remote kết nối → Mở màn hình Player TV
  ...
  void dispose() {
    ...
    final wsServer = Provider.of<WsServerService>(context, listen: false);
    wsServer.removeListener(_onServerStateChange);
    
    // Thu hồi mDNS service
                ...
                builder: (context, child) {
                  return Row(
                    ...
                    children: List.generate(3, (index) {
                      ...
                      final double value = (sin((_animationController.value * 2 * pi) - (delay * pi)) + 1) / 2;
                      return Container(
